 ...
 *
 *  Developed and Maintained by Philips Applied Technologies
...
import org.droidtv.tv.dlna.IUPnPAVObject;
...
public class UPnPAVObject implements Parcelable, IUPnPAVObject {
    ...
    public String getResourceExtnInfo(final String resExtnID, final int id, final int groupNum, final int componentNum) {
        String value = null;
        final ResourceExtension ext = getResourceExt(resExtnID);
        if (ext != null) {
            switch (id) {
    ...
    public int getResourceValueInt(final int id, final int resNum) {
	 ...
	 /**
     * setByteArrayToStringValue Sets a string parameter from Byte array in the AV object
     * 
     * @param id content parameter id
	 * 
	...
	public static String findDlnaInfo(String p4, String flag){
		...
		if(p4 == null || flag == null ||
		   p4.equals("*") || !p4.contains(flag)) {
			return null;
		}
	...
	/*
	 * Returns the list of all subtitle. It includes ComponentId, Mime-Type and URL for each subtitle
	 * Pattern of String is: <CompId>,<Mime-Type>,<URL>;<CompId>,<Mime-Type>,<URL>;<CompId>,<Mime-Type>,<URL> ...
	 */
	public String getSubtitleList() {
		
		Log.v(TAG,"Subtitle: getSubtitleList, Retreiving Subtitle list for:"+getStringValue(UPnPConstants.UPNP_AV_OBJECT_TITLE));
		String result = "";
		int no = 0;
		int i=0;
		...
		for(i=0; i<components.size(); i++){
			...
			if(compId != null){
				...
				if(compId.endsWith(".srt")||compId.endsWith(".sub")||compId.endsWith(".smi")||compId.endsWith(".sami")){
					if(no > 0){
					    //If it is not the first item, add a ',' before appending to the result
						result += ";";
					}
					result += components.get(i).getComponentResURL()+"?componentId=\""+components.get(i).getComponentID()+"\"&MimeType=\""+components.get(i).getComponentMimeType()+"\"";
					no++;
				}
				else{
				    ...
				    if(compMimeType.equals("text/plain")||compMimeType.equals("text/x-microdvd")||compMimeType.equals("application/x-subrip")||compMimeType.equals("application/x-sami")){
					    if(no > 0){
					        //If it is not the first item, add a ',' before appending to the result
						    result += ";";
					    }
		...
		Log.i(TAG,"Subtitle:getSubtitleList, No of Components:"+components.size()+"Total no of subtitles:"+no+" SubtileList:"+result);
		return result;
	}
}
